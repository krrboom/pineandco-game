...
const DEF = {
  ...
  sm_time:13, sm_thr:11, sm_lo:5.5, sm_drop:0.45, sm_cd:25,
  ou_start:3,
...
const ROWS = [
  ...
  ['sm_thr','흔들림 인식 세기(한 번 셈)',4,20,0.5],
  ['sm_lo','힘 빠짐 기준(이 아래로 내려가야 다음 한 번)',1,10,0.5],
  ['sm_drop','다음 한 번을 받는 기준(최고치 대비)',0.2,0.9,0.05],
  ['sm_cd','잡음 거르는 최소 간격(ms)',5,100,5],
  ['— ORDER UP! —','ou'],
...
const HOOKS = [
  ...
  ['    const SM_HI=11, SM_LO=5.5, SM_DROP=0.45, SM_MIN=25;',
   "    const SM_HI=TV('sm_thr',11), SM_LO=TV('sm_lo',5.5), SM_DROP=TV('sm_drop',0.45), SM_MIN=TV('sm_cd',25);"],
